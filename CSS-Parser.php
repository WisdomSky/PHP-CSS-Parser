...
class CSSParser {
	...
	private static function _parse($str){
		$css = new Grammar("MAIN",
			array(
					"L_BRACE"			=>		new 	StringParser("{"),
					"R_BRACE"			=>		new 	StringParser("}"),
					"COLON"				=>		new 	StringParser(":"),
					"SEMI_COLON"		=>		new 	StringParser(";"),
					"WHITESPACE"		=>		new  	 RegexParser("#^[ \n\r\t]*#"),
					"PROPVAL_CHARS"		=> 		new 	 RegexParser("#^[^\{\};:]#"),
					"PROPVAL_STRING"	=>		new	GreedyStarParser("PROPVAL_CHARS",
														function() {
															 return implode("", func_get_args()); 
														}
												    ),
					"SELECTOR_CHARS"	=> 		new 	 RegexParser("#^[^\{\};]#"),
					"SELECTOR_STRING"	=>		new	GreedyStarParser("SELECTOR_CHARS",
														function() {
															 return implode("", func_get_args()); 
														}
												    ),							 
					"PROP_VAL"			=>		new 	  ConcParser(
																array("WHITESPACE","PROPVAL_STRING","WHITESPACE","COLON","WHITESPACE","PROPVAL_STRING","WHITESPACE"),
																function($w,$prop,$w2,$colon,$w3,$value,$w4){
																	return array($prop => $value);
																}
														  ),
					"CON_PROP_VAL"		=> 		new 	  ConcParser(
														  	array("SEMI_COLON","PROP_VAL"),
														  	function($s,$propval){
														  		return $propval;
														  	}
														  ),
					"CON_PROP_VAL_LIST" =>		new GreedyStarParser("CON_PROP_VAL",
														function() {
															$list = array();
															foreach(func_get_args() as $propval) {
																$keys = array_keys($propval);
																$vals = array_values($propval);
																$list[$keys[0]] = $vals[0];
															}
															return $list;
														}
					...
					"FULL_PROP_VAL"		=>		new 	  ConcParser(
															array("PROP_VAL", "CON_PROP_VAL_LIST"),
															function($propval, $list) {
																$keys = array_keys($propval);
																$vals = array_values($propval);
																$list[$keys[0]] = $vals[0];
																return $list;
					...
					"FULL_PROP_VAL_END" =>		new 	  ConcParser(
															array("FULL_PROP_VAL", "SEMI_COLON", "WHITESPACE"),
															function($propval, $s, $w) {
																return $propval;
														  	}
														  ),
					"FULL_PROP_VAL_OR" =>		new    LazyAltParser(
															array("FULL_PROP_VAL_END", "FULL_PROP_VAL")
													   ),
					"BLOCK"				=>		new 	  ConcParser(
																array("WHITESPACE","SELECTOR_STRING","WHITESPACE","L_BRACE","FULL_PROP_VAL_OR","R_BRACE","WHITESPACE"),
																function($w,$label,$w2,$lb,$propvals,$rb,$w3){
																	return array($label => $propvals);
																}
															),
					"MAIN"				=>		new GreedyStarParser("BLOCK",
														function() {
															$list = array();
															foreach(func_get_args() as $propval) {
																$keys = array_keys($propval);
